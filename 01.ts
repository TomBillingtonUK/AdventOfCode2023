...
function replaceNumberString (regex: RegExp, value: string, searchValue: string, replaceValue: string) : string{
...
data.forEach((value: string) => {
  ...
  // Covert the letters numbers into numbers
  let processedValue = "";
  ...
  characters.forEach(character => {
    buffer += character;
    let updatedBuffer  = ""
    updatedBuffer  = replaceNumberString(/zero/, buffer, "zero", "0");
    updatedBuffer  = replaceNumberString(/one/, updatedBuffer, "one", "1");
    updatedBuffer  = replaceNumberString(/two/, updatedBuffer, "two", "2");
    updatedBuffer  = replaceNumberString(/three/, updatedBuffer, "three", "3");
    updatedBuffer  = replaceNumberString(/four/, updatedBuffer, "four", "4");
    updatedBuffer  = replaceNumberString(/five/, updatedBuffer, "five", "5");
    updatedBuffer  = replaceNumberString(/six/, updatedBuffer, "six", "6");
    updatedBuffer  = replaceNumberString(/seven/, updatedBuffer, "seven", "7");
    updatedBuffer  = replaceNumberString(/eight/, updatedBuffer, "eight", "8");
    updatedBuffer  = replaceNumberString(/nine/, updatedBuffer, "nine", "9");

    if(updatedBuffer !== buffer) {
      processedValue += updatedBuffer
      buffer = "";
    }
  });
  processedValue += buffer

  const numbers = value.match(/\d{1}/g)
  ...
  const fullNumber = Number.parseInt(`${firstNumber}${lastNumber}`)
  total += fullNumber;
})
